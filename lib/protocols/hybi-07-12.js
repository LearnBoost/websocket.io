...
Parser.prototype.op1 = function (data) {
  var self = this;

  function finish (mask, data) {
  ...
  if (firstLength < 126) {
    ...
    this.expect('Length', 2, function (data) {
      expectData(util.unpack(data));
    });
  } else if (firstLength == 127) {
    this.expect('Length', 8, function (data) {
      if (util.unpack(data.slice(0, 4)) != 0) {
        this.error('packets with length spanning more than 32 bit is currently not supported');
        return;
      }
...
Parser.prototype.op2 = function (data) {
  var self = this;
...
Parser.prototype.op8 = function (data) {
  this.emit('close');
  this.reset();
};
...
Parser.prototype.op9 = function (data) {
...
Parser.prototype.processPacket = function (data) {
  ...
  if (!this['op' + this.state.opcode]) {
    this.error('no handler for opcode ' + this.state.opcode);
  } else {
    this['op' + this.state.opcode](data);
  }
};
