...
Server.prototype.handleUpgrade = function (req, socket, head) {
  ...
  if (! this.checkRequest(req)) {
    ...
    return this;
  } else if (this.options.originCheck) {
    
    //need to check for origin 
    this.options.originCheck(req.headers['origin'], function(checked) {
    ...
    //no need to check for the origin
    acceptClient();
  }
  ...
  function acceptClient() {
    ...
    var client = self.createClient(req);
  ...
  function denyClient() {
    // attach the legacy `head` property to reques
    req.head = head;

    self.createClient(req).end();
  }
